  ...
  return (
    ...
    >
      <div className="aspect-[2/3] overflow-hidden relative shrink-0">
        ...
        {item.topOrder && rank === undefined && (
          <div className="absolute top-0 left-0 bg-gradient-to-br from-amber-300 to-amber-600 text-slate-950 font-black px-2 sm:px-3 py-0.5 sm:py-1 rounded-br-xl shadow-xl z-10 text-[10px] sm:text-xs flex items-center gap-0.5 border-b border-r border-amber-300/30">
            <span className="text-[8px] sm:text-[10px] opacity-70">TOP</span>
            {item.topOrder}
          </div>
        )}
        ...
        <div
          className={`absolute bottom-2 left-2 px-1.5 sm:px-2 py-0.5 rounded-md text-[8px] sm:text-[9px] font-black uppercase tracking-wider text-white shadow-lg z-10 ${getStatusColor(item.status)}`}
        >
          {item.status}
        </div>
      </div>
      ...
      <div className="p-2 sm:p-2.5 flex flex-col flex-1 justify-between gap-1.5">
        <div>
          <h3 className="text-[10px] sm:text-xs font-bold text-white line-clamp-1 group-hover:text-indigo-400 transition-colors">{item.title}</h3>
          <p className="text-[7px] sm:text-[9px] text-slate-500 mt-0.5 uppercase tracking-widest font-bold">{item.category}</p>
        </div>
