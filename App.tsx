...
const App: React.FC = () => {
  ...
  const filteredItems = useMemo(() => {
    let result = items.filter(item => {
      ...
      const matchGenre = filterGenre === 'All' || 
        item.genres.some(g => g.toLowerCase().trim() === filterGenre.toLowerCase().trim());
  ...
  const exportDatabase = () => {
    ...
    linkElement.setAttribute('download', `oni_db_${new Date().toISOString().split('T')[0]}.json`);
    linkElement.click();
  };
  ...
  const importDatabase = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      } catch (err) { alert("Lỗi file JSON! Vui lòng kiểm tra lại file."); }
    };
  ...
  const NavLinks = ({ mobile = false }: { mobile?: boolean }) => {
    ...
    return (
      <>
        <div onClick={() => { setViewMode('all'); mobile && setIsMobileMenuOpen(false); }} className={`${linkClass} ${viewMode === 'all' ? activeClass : ''}`}>{mobile && <i className="fa-solid fa-house w-6"></i>} Trang chủ</div>
        {separator}
        <div onClick={() => { setViewMode('ranking'); mobile && setIsMobileMenuOpen(false); }} className={`${linkClass} ${viewMode === 'ranking' ? activeClass : ''}`}>{mobile && <i className="fa-solid fa-trophy w-6"></i>} Xếp hạng</div>
        {separator}
        <div onClick={() => { setViewMode('history'); mobile && setIsMobileMenuOpen(false); }} className={`${linkClass} ${viewMode === 'history' ? activeClass : ''}`}>{mobile && <i className="fa-solid fa-clock-rotate-left w-6"></i>} Lịch sử</div>
        {separator}
        <div onClick={() => { setViewMode('settings'); mobile && setIsMobileMenuOpen(false); }} className={`${linkClass} ${viewMode === 'settings' ? activeClass : ''}`}>{mobile && <i className="fa-solid fa-database w-6"></i>} Dữ liệu (JSON)</div>
      </>
    );
  };
  ...
  const FilterChip = ({ category, label, icon }: { category: MediaCategory | 'All', label: string, icon: string }) => {
    ...
    return (
      ...
      >
        ...
        {label}
        <span className={`ml-1 px-1.5 py-0.5 rounded-md text-[10px] ${active ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-600'}`}>
  ...
  return (
    <div className="min-h-screen pb-24 bg-slate-950 text-slate-200 selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      {/* Mobile Sidebar */}
      <div className={`fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMobileMenuOpen(false)}></div>
      <aside className={`fixed top-0 left-0 z-[60] h-full w-72 bg-slate-900 border-r border-white/5 transition-transform duration-300 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-white/5 flex justify-between items-center">
            <span className="text-lg font-bold text-white tracking-tighter">ONI<span className="text-indigo-400">SCENARY</span></span>
            <button onClick={() => setIsMobileMenuOpen(false)} className="text-slate-400 p-2"><i className="fa-solid fa-xmark text-xl"></i></button>
        </div>
        <nav className="mt-4 flex flex-col">
            <NavLinks mobile />
            <div className="h-px bg-white/5 my-4 mx-6"></div>
            <div onClick={() => { setIsProfileModalOpen(true); setIsMobileMenuOpen(false); }} className="flex items-center gap-4 px-6 py-4 text-slate-300 hover:text-white hover:bg-white/5 transition-all text-lg font-medium border-l-4 border-transparent"><i className="fa-solid fa-user-circle w-6"></i> Tài khoản</div>
            <div onClick={handleLogout} className="flex items-center gap-4 px-6 py-4 text-rose-400 hover:text-rose-300 hover:bg-rose-500/5 transition-all text-lg font-medium border-l-4 border-transparent"><i className="fa-solid fa-right-from-bracket w-6"></i> Đăng xuất</div>
        </nav>
      </aside>
      ...
      <header className="sticky top-0 z-40 glass-morphism h-16 border-b border-white/5 px-4 sm:px-6 flex items-center justify-between gap-2 sm:gap-4">
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden text-slate-300 hover:text-white p-2 -ml-2"><i className="fa-solid fa-bars-staggered text-lg"></i></button>
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => {setViewMode('all'); setFilterCategory('All'); setFilterGenre('All'); setSearchQuery(''); setCurrentPage(1);}}>
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-600 rounded-lg flex items-center justify-center shadow-lg shadow-indigo-500/40"><i className="fa-solid fa-clapperboard text-white text-sm sm:text-base"></i></div>
            <span className="text-lg sm:text-xl font-bold tracking-tighter text-white block">ONI<span className="text-indigo-400">SCENARY</span></span>
          </div>
        </div>
        
        <nav className="hidden md:flex items-center gap-4 mr-auto ml-8"><NavLinks /></nav>
        ...
        <div className={`flex-1 max-w-md ${showMobileSearch ? 'absolute inset-x-0 top-0 h-16 px-4 bg-slate-900 z-50 flex items-center gap-2' : 'hidden lg:block'}`}>
          <div className="relative group w-full">
            ...
            />
            {searchQuery && (
               <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white"><i className="fa-solid fa-circle-xmark text-xs"></i></button>
            )}
          </div>
          {showMobileSearch && (
            <button onClick={() => setShowMobileSearch(false)} className="text-slate-400 text-sm font-medium whitespace-nowrap">Hủy</button>
          )}
        </div>

        <div className="flex items-center gap-1 sm:gap-3">
          <button onClick={() => setShowMobileSearch(true)} className="lg:hidden w-9 h-9 flex items-center justify-center text-slate-400 hover:text-white"><i className="fa-solid fa-magnifying-glass"></i></button>
          <Button onClick={() => { setSelectedItem(null); setIsModalOpen(true); }} className="rounded-full px-3 sm:px-5 py-1.5 sm:py-2 text-sm">
            <i className="fa-solid fa-plus"></i> <span className="hidden sm:inline">Thêm</span>
          </Button>
          <div className="relative" ref={userMenuRef}>
            <button onClick={() => setIsUserDropdownOpen(!isUserDropdownOpen)} className={`w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-slate-800 border-2 transition-all overflow-hidden ${isUserDropdownOpen ? 'border-indigo-500 scale-105 sm:scale-110' : 'border-white/10 hover:border-white/30'}`}>
            ...
            <div className={`absolute right-0 mt-3 w-48 sm:w-56 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl transition-all duration-200 z-50 overflow-hidden ${isUserDropdownOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
              <div className="p-4 border-b border-white/5 bg-white/5"><p className="text-xs sm:text-sm font-bold text-white truncate">{user.username}</p></div>
              <div className="p-2">
                <button onClick={() => { setIsProfileModalOpen(true); setIsUserDropdownOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm text-slate-300 hover:bg-white/10 transition-colors text-left"><i className="fa-solid fa-user-gear"></i> Hồ sơ</button>
                <button onClick={() => { setViewMode('settings'); setIsUserDropdownOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm text-slate-300 hover:bg-white/10 transition-colors text-left"><i className="fa-solid fa-database"></i> Dữ liệu</button>
                <div className="h-px bg-white/5 my-2"></div>
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm text-rose-400 hover:bg-rose-500/10 transition-colors text-left"><i className="fa-solid fa-right-from-bracket"></i> Đăng xuất</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {viewMode === 'settings' ? (
        <main className="px-4 max-w-4xl mx-auto py-8 sm:py-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex flex-col items-center mb-8 sm:mb-12">
            <div className="w-14 h-14 sm:w-16 sm:h-16 bg-slate-800 rounded-2xl flex items-center justify-center border border-white/10 mb-4 shadow-xl"><i className="fa-solid fa-database text-2xl sm:text-3xl text-indigo-400"></i></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
            <div className="bg-slate-900/50 border border-white/5 rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 hover:border-indigo-500/30 transition-all shadow-xl flex flex-col items-center text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-indigo-500/10 rounded-full flex items-center justify-center mb-4 sm:mb-6"><i className="fa-solid fa-download text-indigo-400 text-xl sm:text-2xl"></i></div>
              ...
              <Button variant="primary" onClick={exportDatabase} className="w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl text-base sm:text-lg"><i className="fa-solid fa-file-export"></i> Tải về ngay</Button>
            </div>
            <div className="bg-slate-900/50 border border-white/5 rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 hover:border-emerald-500/30 transition-all shadow-xl flex flex-col items-center text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-emerald-500/10 rounded-full flex items-center justify-center mb-4 sm:mb-6"><i className="fa-solid fa-upload text-emerald-400 text-xl sm:text-2xl"></i></div>
              ...
              <Button variant="secondary" onClick={() => fileInputRef.current?.click()} className="w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl text-base sm:text-lg border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10"><i className="fa-solid fa-file-import"></i> Chọn file</Button>
            </div>
          </div>
          <div className="mt-8 sm:mt-12 flex justify-center">
            <Button variant="ghost" onClick={() => setViewMode('all')} className="rounded-xl px-6 sm:px-8 py-3"><i className="fa-solid fa-arrow-left"></i> Quay lại Home</Button>
          </div>
        </main>
      ) : (
            ...
            <section className="px-4 sm:px-6 mt-4 sm:mt-8">
              <div className="relative overflow-hidden rounded-2xl sm:rounded-[32px] bg-gradient-to-br from-slate-800 to-slate-900 border border-white/5 p-5 sm:p-8 shadow-2xl min-h-[120px] sm:min-h-[140px] flex items-center">
                <div className="relative z-10">
                    <h1 className="text-2xl sm:text-4xl font-extrabold text-white leading-tight tracking-tighter">Welcome, <span className="text-indigo-400">Onis</span></h1>
          ...
          <div className="px-4 sm:px-6 mt-6 sm:mt-8">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                ...
                {viewMode !== 'all' && <button onClick={() => setViewMode('all')} className="text-[11px] sm:text-xs text-indigo-400 font-bold hover:underline">Quay lại Home</button>}
              </div>
          ...
          <main className="px-4 sm:px-6 mt-4 sm:mt-6 min-h-[400px]">
            {filteredItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 sm:py-32 bg-slate-900/30 rounded-[24px] sm:rounded-[32px] border border-dashed border-white/5 text-slate-500 text-center px-6">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-800 rounded-[20px] sm:rounded-3xl flex items-center justify-center mb-4 sm:mb-6 shadow-lg"><i className="fa-solid fa-folder-open text-2xl sm:text-3xl opacity-20"></i></div>
                <p className="text-base sm:text-lg font-medium text-slate-300">Không tìm thấy phim phù hợp...</p>
                <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-6">
                    <Button variant="primary" className="rounded-xl px-5 sm:px-6 py-2 text-sm" onClick={() => { setIsModalOpen(true); setSelectedItem(null); }}>Thêm phim mới</Button>
                    {(filterCategory !== 'All' || filterGenre !== 'All') && <Button variant="ghost" className="rounded-xl px-5 sm:px-6 py-2 text-sm" onClick={() => {setFilterCategory('All'); setFilterGenre('All');}}>Hiện tất cả</Button>}
                </div>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-6 md:gap-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
                  {pagedItems.map((item, index) => (
                    <MovieCard 
                      ...
                      onClick={(item) => { setSelectedItem(item); setIsModalOpen(true); }}
      ...
      <AddMediaModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setSelectedItem(null); }} onSave={handleSave} editingItem={selectedItem} allItems={items} />
      ...
      {selectedItem && isModalOpen && (
        <div className="fixed bottom-24 sm:bottom-6 left-0 right-0 z-[70] px-4 flex justify-center pointer-events-none animate-in slide-in-from-bottom-10 duration-300">
            <div className="bg-slate-900/95 backdrop-blur-2xl border border-white/10 rounded-[20px] sm:rounded-[24px] p-2 flex gap-2 shadow-2xl pointer-events-auto">
                <Button variant="danger" onClick={() => handleDelete(selectedItem.id)} className="text-xs px-4 sm:px-6 py-2 sm:py-2.5 rounded-[14px] sm:rounded-[18px]">Xóa khỏi kho</Button>
            </div>
        </div>
      )}
    </div>
